...
impl Solution {
    pub fn find_difference(nums1: Vec<i32>, nums2: Vec<i32>) -> Vec<Vec<i32>> {
        ...
        let ls: HashSet<i32> = HashSet::from_iter(nums1);
        let rs: HashSet<i32> = HashSet::from_iter(nums2);
        let mut lv: Vec<i32> = vec![];
        let mut rv: Vec<i32> = vec![];
        for i in ls.difference(&rs) {
            lv.push(*i);
        }
        for i in rs.difference(&ls) {
            rv.push(*i);
        }
